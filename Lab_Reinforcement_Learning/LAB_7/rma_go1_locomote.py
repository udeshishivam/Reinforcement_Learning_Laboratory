...
def go1_rma_default_config() -> config_dict.ConfigDict:
  return config_dict.create(
      ...
      reward_config=config_dict.create(
          scales=config_dict.create(
              # Tracking.
              track_forward_vel_rma=20,
              
              # Penalties 
              lateral_mov_rot_rma=-21,
              work_rma=-0.002,
              gnd_impact_rma= 0, #-0.02,
...
class LocomotionRMAEnv(go1_base.Go1Env):
  ...
  def _post_init(self) -> None:
    ...
    self._feet_body_id = np.array( #TODO(drewskis): uhhhhhhh
  ...
  def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
    ...
    # print("These are the metrics", state.metrics)
  ...
  def _get_obs(
    ...
    # TODO(student): Implement the agent state. The state should include:
    #    1. the local linear velocity (3, )
    #    2. the local angular velocity - from gyro (3, )
    #    3. the gravity vector (3, )
    #    4. the joint angles, normalized by the default pose (12, )
    #    5. the joint velocities (12, )
    #    6. the last action (12, )
    # Don't worry about adding small gaussian noise to these measurement in this lab.
    # HINT: Many of these quantities are already implemented as functions in the parent class: `go1_base.Go1Env`
    state = jp.hstack([
        # Fill this in 
    ])
    ...
    FLOOR_GEOM_ID = 0
    TORSO_BODY_ID = 1
    # TODO(student): Implement the agent privileged state. It's up to you to decide
    # exactly what information to put in the privileged environment state. Try to make it at least 6 dims though
    # I personally keep it simple by including:
    #   1. The surface friction of the ground (1, )
    #   2. The mass of the torso (1, )
    #   3. The info["last_contact"] binary vector for each foot (4, )
    # HINT: You can refer to the go1_randomizer (https://github.com/google-deepmind/mujoco_playground/blob/main/mujoco_playground/_src/locomotion/go1/randomize.py)
    # to remember how to get these quantities.     
  ...
  def _reward_forward_vel_rma(self, 
      ...
      return jp.minimum(0.35, local_vel[0])
  ...
  def _cost_gnd_impact_rma(self,
    ...
    #   jax.debug.print("This is f_gnd {}", f_gnd)
    #   jax.debug.print("This is last f_gnd {}", last_f_gnd)
