...
class EECS598RSLRLBraxWrapper(VecEnv):
  ...
  def __init__(
    ...
    if isinstance(obs_shape, dict):
      ...
      self.num_obs = obs_shape["state"][0]
      self.num_privileged_obs = obs_shape["privileged_state"][0]
    else:
  ...
  def step(self, action):
    ...
    # TODO: used to not have critic_obs I think
    return obs, critic_obs, reward, done, info_ret
  ...
  def get_privileged_observations(self):
    # NOTE: This was the new requirements. 
    if self.asymmetric_obs:
        _, priv_obs = self.reset_with_critic_obs()
    else:
        priv_obs = None
