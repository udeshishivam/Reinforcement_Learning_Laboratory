...
"""
TODO(student): This is an example of a customized environment based on the Go1 environment from playground.

The original implementation is taken from:
https://github.com/google-deepmind/mujoco_playground/blob/main/mujoco_playground/_src/locomotion/go1/joystick.py
"""
...
def go1_locomote_default_config() -> config_dict.ConfigDict:
...
class LocomotionJoystickEnv(go1_base.Go1Env):
  """Track a joystick command."""

  def __init__(
      self,
      task: str = "flat_terrain",
      config: config_dict.ConfigDict = go1_locomote_default_config(),
  ...
  def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
    ...
    self._maybe_sample_env_params(state)
  ...
  def _get_obs(
    ...
    state = jp.hstack([
        ...
        info["last_act"],  # 12
        info["command"],  # 3
        # vvv BELOW IS SOME PRIVILEGED INFORMATION vvv
  ...
  def _maybe_sample_env_params(self, state: mjx_env.State) -> mjx_env.State:
      '''This function samples new env parameters such as:
        1. Ground Friction
        2. ...
      '''
...
# TODO(student): Make sure to register your custom environment
envs.register_environment('custom_go1_locomote', LocomotionJoystickEnv)